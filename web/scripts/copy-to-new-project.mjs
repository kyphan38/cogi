// ---------------------------------------------------------------------------
// cogi - Chép dữ liệu từ project cũ sang project mới.
//
//   Nguồn: kyphan38-apps / database 'cogi-db'        (service account cũ)
//   Đích:  kyphan38-cogi-app / database '(default)'  (service account mới)
//
...
//
// Credential cũ: lấy từ biến OLD_FIREBASE_ADMIN_*, nếu không có thì đọc file
// backup /tmp/cogi.env.bak (bước 2 của plan đã tạo). Đổi chỗ bằng OLD_ENV_FILE.
//
// Lưu ý: project cũ phải còn gói Blaze, vì 'cogi-db' là named database và
// Admin SDK đọc named database thì đòi billing.
//
...
const SOURCE_DB = "cogi-db";

// Chỉ 14 collection của cogi. Doc gốc users/{uid} từng dùng chung với
// logi/noda, nên tuyệt đối không quét mù mọi subcollection.
const COLLECTIONS = [
  "actions",
  "activeMathSessions",
  "aiArtifacts",
...
const OLD_ENV_BACKUP = process.env.OLD_ENV_FILE ?? "/tmp/cogi.env.bak";
...
function readOldEnv() {
  ...
  if (ADMIN_KEYS.every((k) => fromEnv[k])) return fromEnv;
  ...
  for (const line of text.split("\n")) {
    ...
    if (!m || !ADMIN_KEYS.includes(m[1])) continue;
    let v = m[2].trim();
    if (v.startsWith('"') && v.endsWith('"')) v = v.slice(1, -1);
    out[m[1]] = v;
  }
  for (const k of ADMIN_KEYS) if (!out[k]) throw new Error(`${OLD_ENV_BACKUP} thiếu ${k}`);
  return out;
}
...
if (old.FIREBASE_ADMIN_PROJECT_ID === process.env.FIREBASE_ADMIN_PROJECT_ID) {
  throw new Error(
    `Project nguồn và đích trùng nhau (${old.FIREBASE_ADMIN_PROJECT_ID}). ` +
      "Kiểm tra .env.local đã đổi sang project mới chưa.",
  );
}
...
console.log(
  `Đích:  ${process.env.FIREBASE_ADMIN_PROJECT_ID} / (default) / users/${TO_UID}`,
);
...
// Doc gốc users/{uid}: đếm riêng, không tính vào tổng subcollection.
const userSnap = await srcUser.get();
if (userSnap.exists) {
  if (COMMIT) await dstUser.set(userSnap.data(), { merge: true });
  console.log(`  users/${FROM_UID} (doc gốc): có, chép riêng`);
} else {
...
for (const name of COLLECTIONS) {
  ...
  for (const doc of snap.docs) {
    batch.set(dstUser.collection(name).doc(doc.id), doc.data());
    n += 1;
    if (n === BATCH_SIZE) {
      await batch.commit();
      batch = dst.batch();
...
console.log(
  COMMIT
    ? `\nĐã ghi ${total} doc (chưa kể doc gốc).`
    : `\nChạy khô: sẽ ghi ${total} doc (chưa kể doc gốc).`,
);
process.exit(0);
