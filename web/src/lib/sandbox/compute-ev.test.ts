...
describe("computeSandboxEv", () => {
  ...
  it("does not flag ruin when the worst loss is within reserves", () => {
    const result = computeSandboxEv(
      baseInput({ oneShot: true, branches: [{ id: "b1", label: "Loss", probability: 0.1, payoff: -1_000_000 }], reserves: 2_000_000 }),
    );
    expect(result.ruinFlag).toBe(false);
  });
  ...
  it("rounds EV to 2 decimal places", () => {
    const result = computeSandboxEv(
      baseInput({
        fixedCost: 0,
        branches: [{ id: "b1", label: "x", probability: 1 / 3, payoff: 100 }],
      }),
    );
    expect(Number.isInteger(result.ev * 100)).toBe(true);
  });
});
