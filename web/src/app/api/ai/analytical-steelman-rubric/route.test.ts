...
function makeRequest(body: unknown) {
  return new Request("http://localhost/api/ai/analytical-steelman-rubric", {
    method: "POST",
...
const validExercise = {
  ...
  validPoints: [],
  userHighlights: [],
  analyticalVariant: "steelman" as const,
...
describe("POST /api/ai/analytical-steelman-rubric", () => {
  ...
  it("returns 400 when exercise type is not analytical", async () => {
    authOk();
    const res = await POST(makeRequest({ exercise: { type: "generative" } }));
    expect(res.status).toBe(400);
  });

  it("returns 400 when analyticalVariant is not steelman", async () => {
    authOk();
    const res = await POST(
      makeRequest({ exercise: { ...validExercise, analyticalVariant: "highlight_tag" } }),
    );
    expect(res.status).toBe(400);
  });
